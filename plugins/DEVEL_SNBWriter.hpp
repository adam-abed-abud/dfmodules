...
private:
  ...
  int m_num_producers = 10;
  char* m_membuffers[10]; 
  std::vector<SNBHandler> m_snb_handlers;
  std::unique_ptr<AsyncIO>  m_asyncio_ptrs[10];
  std::vector<std::future<void>> m_async_thread_vec;
