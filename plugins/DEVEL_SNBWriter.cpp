...
SNBWriter::do_conf(const data_t& payload)
{
  ...
  // Reserve aligned memory for SNB data store to write.
  for (int i=0; i < m_num_producers; i++) {
    //m_membuffers[i] = nullptr;
    AsyncIO::memalloc(reinterpret_cast<void**>(&m_membuffers[i]), PAGE_SIZE, m_alloc_size);
  }
...
SNBWriter::do_work(std::atomic<bool>& running_flag)
{
  ...
  // Prepare and initialize SNB data store
  //for (int i=0; i < m_num_producers; i++) {
   //SNBHandler snb_store("/mnt/micron1/output_link" + std::to_string(i) + ".bin", io_size_, false); 
   //snb_store.init();
   //m_snb_handlers.push_back(snb_store);
  //}
  SNBHandler snb_store("/mnt/micron1/output_link" + std::to_string(0) + ".bin", io_size_, false);  
  ...
  while (running_flag.load()) {
    ...
    for (const auto& frag_ptr : frag_vec) {
      ...
      //m_async_thread_vec.push_back(std::async(std::launch::async, &SNBHandler::store, &m_snb_handlers[link_number], m_asyncio_ptrs[link_number], m_membuffers[link_number], false, 2));
      snb_store.store(m_membuffers[0], false, 2 );
      //std::async(std::launch::async, &SNBHandler::store, &m_snb_handlers[0], m_membuffers[0], false, 2);
      link_number++;
    }
