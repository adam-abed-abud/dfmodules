...
SNBWriter::do_work(std::atomic<bool>& running_flag)
{
  ...
  for (int i=0; i < m_num_producers; i++) {
    ...
    snb_handlers.push_back(new SNBHandler("/mnt/micron1/output_link" + std::to_string(i) + ".bin", io_size_, false));
  ...
  while (running_flag.load()) {
    ...
    */
    ...
    for (const auto& frag_ptr : frag_vec) {
      ...
      //snb_handlers[0]->store(membuffer_, false, 2 );
      for (int thread = 0; thread < m_num_producers; thread++) {
        std::async(std::launch::async, &SNBHandler::store, snb_handlers[thread], membuffer_, false, thread);  
      }
      link_num++;
    }
