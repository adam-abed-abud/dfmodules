...
SNBWriter::do_conf(const data_t& payload)
{
  ...
  AsyncIO::memalloc(reinterpret_cast<void**>(&membuffer_), PAGE_SIZE, 10485760);
...
SNBWriter::do_work(std::atomic<bool>& running_flag)
{
  ...
  while (running_flag.load()) {
    ...
    for (const auto& frag_ptr : frag_vec) {
      if (total_mem_size < io_size_) {
        void* data_block = frag_ptr->get_storage_location();
        size_t data_block_size = frag_ptr->get_size();
        memcpy(membuffer_ + total_mem_size, (char*)data_block, data_block_size);
        //membuffer_ += data_block_size + 1;
        total_mem_size += data_block_size;
      } else {      
        snb_data_store.store(fd, membuffer_, false, 2 );
        total_mem_size = 0;
      }
...
*/ 
    ...
    for (const auto& frag_ptr : frag_vec) {
      void* data_block = frag_ptr->get_storage_location();
      size_t data_block_size = frag_ptr->get_size();
      memcpy(membuffer_ + total_block_size, (char*)data_block, data_block_size);
      total_block_size += data_block_size;
    }
    // if greater than 0.8*size of block, then write
    // block in this case is 10 MB
    if (total_block_size > 2097152) { 
      snb_data_store.store(fd, membuffer_, false, 2 );
      total_block_size = 0; 
    }
